namespace :javascript do
  ...
  build_task.prereqs << :install unless ENV["SKIP_YARN_INSTALL"] || ENV["SKIP_BUN_INSTALL"] || ENV["SKIP_DENO_INSTALL"]
end
...
module Jsbundling
  module Tasks
    ...
    def build_command
      ...
      when :deno then "deno task build"
    ...
    def tool_determined_by_config_file
      ...
      when File.exist?("deno.lock")         then :deno
    ...
    def tool_determined_by_executable
      %i[ bun deno yarn pnpm npm ].each do |exe|
